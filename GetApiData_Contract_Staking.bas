' New module in Excel add button
Sub GetApiData_Contract_Staking()
    Dim httpRequest As Object
    Set httpRequest = CreateObject("WinHttp.WinHttpRequest.5.1")
    
    ' Définissez l'URL de l'API pour le staking sur Meria
    Dim apiUrl As String
    apiUrl = "https://api.meria.com/v1/stakings"
...
' Message dans la barre de statut
    ...
    If httpRequest.Status = 200 Then
       ...
       ' Vérifier si la propriété "success" est vraie
        ...
        ' Ecrire les donées dans Excel
            With ThisWorkbook.Sheets("apijm")
                .Range(Cells(3, 5), .Cells(dataCount + 1, 12)).Value = datas
            
            ' Écriture des détails de variations
            .Range(.Cells(3, 15), .Cells(varCount + 1, 19)).Value = varDetails
            
            ' Écriture des détails de crédits
            .Range(.Cells(3, 22), .Cells(creditCount + 1, 25)).Value = creditDetails
